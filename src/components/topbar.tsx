...
export default function TopBar() {
  ...
  useEffect(() => {
    if (!getToken()) return;
    apiFetchClient<{ name?: string; email?: string }>("/auth/me")
      .then(setMe)
      .catch(() => setMe(null));
  }, []);
  ...
  return (
    <div className="flex items-center justify-between gap-4 px-5 pt-6 sm:px-8 md:pt-8" style={{ paddingTop: "calc(env(safe-area-inset-top) + 24px)" }}>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#B4FF39] text-sm font-bold text-black">
          {initials || "?"}
        </div>
      </div>
    </div>
